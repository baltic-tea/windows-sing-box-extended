...
if "!DEST_FILE!"=="" (
    if "!IS_ADMIN!"=="1" (
        ...
        if "%LOCALAPPDATA%"=="" (
            echo %R%[^^!] ОШИБКА: LOCALAPPDATA не задан. Укажите путь через -DestFile.%N%
            exit /b 1
        )
...
if %ERRORLEVEL% == 0 (
    if "!IS_ADMIN!"=="0" (
        echo %R%[^^!] ОШИБКА: Для установки в Program Files запустите скрипт от имени администратора.%N%
        exit /b 1
    )
)
...
if "!ARCH_SUFFIX!"=="" (
    ...
    if /i "!HOST_ARCH!"=="AMD64" (
        ...
        echo %R%[^^!] ОШИБКА: Архитектура !HOST_ARCH! не поддерживается.%N%
        exit /b 1
    )
)
...
echo %R%[^^!] ОШИБКА: Неизвестное действие "%ACTION%"%N%
exit /b 1
...
echo %C%[*] Получаю список последних версий...%N%
...
if "!count!"=="0" (
    echo %R%[^^!] ОШИБКА: Не удалось получить список стабильных релизов.%N%
    exit /b 1
)
echo   %Y%0%N%) Отмена
echo.
...
:prompt_version
set "choice="
set /p "choice=%C%[?] Выберите версию (0-!count!): %N%"
if "!choice!"=="0" (
    echo %G%[*] Установка отменена.%N%
    exit /b 0
)
if "!choice!"=="" goto prompt_version
set "SELECTED_TAG=!REL_%choice%!"
if "!SELECTED_TAG!"=="" (
    echo %R%[^^!] Неверный выбор.%N%
    goto prompt_version
)
...
echo.
if "!CURRENT_VER!"=="" (
    set "disp_curr=не установлен"
) else (
    set "disp_curr=!CURRENT_VER!"
)
echo %C%[*] Текущая: %Y%!disp_curr!%C% ^| Выбранная: %Y%!SELECTED_VER!%N%

if "!CURRENT_VER!"=="!SELECTED_VER!" (
    if "!FORCE_REINSTALL!"=="1" (
        echo %Y%[^^!] Эта версия уже установлена. Выполняю переустановку...%N%
    ) else (
        echo %Y%[^^!] Эта версия уже установлена.%N%
        set /p "reins_choice=%C%[?] Переустановить? [y/N]: %N%"
        if /i "!reins_choice!"=="y" goto do_install
        if /i "!reins_choice!"=="yes" goto do_install
        if /i "!reins_choice!"=="д" goto do_install
        if /i "!reins_choice!"=="да" goto do_install

        echo %G%[+] Уже установлена выбранная версия. Переустановка не требуется.%N%
...
:do_install
echo %C%[*] Ищу ссылку на скачивание для версии !SELECTED_TAG!...%N%
...
if "!DOWNLOAD_URL!"=="" (
    echo %R%[^^!] ОШИБКА: Файл для архитектуры '%ARCH_SUFFIX%' не найден.%N%
    exit /b 1
)
...
if %ERRORLEVEL% neq 0 (
    echo %R%[^^!] ОШИБКА: Недостаточно свободного места в папке назначения.%N%
    exit /b 1
)
...
echo %C%[*] Скачиваю и устанавливаю...%N%
...
if not exist "!TEMP_DIR!\sing-box.zip" (
    echo %R%[^^!] ОШИБКА: Не удалось скачать файл.%N%
    rd /s /q "!TEMP_DIR!"
    exit /b 1
)
...
if exist "!DEST_FILE!" (
    ...
    echo %C%[*] Backup старого бинарника создан.%N%
)
...
powershell -NoProfile -Command "$ErrorActionPreference = 'Stop'; try { Expand-Archive -LiteralPath $env:ENV_ZIP_PATH -DestinationPath $env:ENV_EXTRACT_DIR -Force; $f = Get-ChildItem -LiteralPath $env:ENV_EXTRACT_DIR -Recurse -Filter 'sing-box.exe' -ErrorAction SilentlyContinue; if (-not $f) { throw 'Файл sing-box.exe не найден в распакованном архиве.' }; if (-not (Test-Path -LiteralPath $env:ENV_DEST_DIR)) { $null = New-Item -ItemType Directory -Path $env:ENV_DEST_DIR -Force }; Copy-Item -LiteralPath $f[0].FullName -Destination $env:ENV_DEST_FILE -Force } catch { $_.Exception.Message | Out-File -LiteralPath $env:ENV_ERR_LOG -Encoding utf8; exit 1 }"

if %ERRORLEVEL% neq 0 (
    echo %R%[^^!] ОШИБКА: Не удалось распаковать или установить бинарник.%N%
    if exist "!TEMP_DIR!\install_error.log" (
        echo %R%Детали ошибки:%N%
...
if "!NO_ADD_TO_PATH!"=="0" (
    if "!ADD_TO_MACHINE_PATH!"=="1" (
        ...
        if "!path_added!"=="1" (
            echo %G%[+] sing-box-extended добавлен в Machine PATH: !DEST_DIR!%N%
        )
    ) else (
        ...
        set /p path_added=<"!TEMP_DIR!\path_res.txt"
        if "!path_added!"=="1" (
            echo %G%[+] sing-box-extended добавлен в User PATH: !DEST_DIR!%N%
        )
    )
)
...
if "!CREATE_SERVICE!"=="1" (
    ...
    if %ERRORLEVEL% neq 0 (
        echo %R%[^^!] ОШИБКА: Не удалось создать службу Windows.%N%
    ) else (
        echo %G%[+] Служба Windows создана: %RESOLVED_SERVICE%%N%
    )
)
...
if "!disp_new!"=="" set "disp_new=н/д"
echo %G%[+] Готово: !disp_curr! -^> !disp_new!%N%

:Action_OfferRun
if "!NO_RUN_PROMPT!"=="1" exit /b 0
if "!RUN_NOW!"=="1" goto start_singbox_process

set /p "run_ans=%C%[?] Запустить sing-box extended сейчас? [y/N]: %N%"
if /i "!run_ans!"=="y" goto start_singbox_process
if /i "!run_ans!"=="yes" goto start_singbox_process
if /i "!run_ans!"=="д" goto start_singbox_process
if /i "!run_ans!"=="да" goto start_singbox_process
exit /b 0
...
if exist "!DEST_FILE!" (
    del /f /q "!DEST_FILE!"
    echo %G%[+] Бинарник удален: !DEST_FILE!%N%
) else (
    echo %Y%[^^!] Бинарник не найден: !DEST_FILE!%N%
)
exit /b 0
...
if "!IS_ADMIN!"=="0" (
    echo %R%[^^!] ОШИБКА: Для управления службами требуются права администратора.%N%
    exit /b 1
)
if not "!IMPORT_CONFIG!"=="" call :ImportConfigHelper
powershell -NoProfile -Command "Stop-Service -Name '%RESOLVED_SERVICE%' -ErrorAction SilentlyContinue"
...
if "!svc_removed!"=="1" (
    echo %G%[+] Служба Windows удалена: %RESOLVED_SERVICE%%N%
) else (
    echo %Y%[^^!] Служба не найдена: %RESOLVED_SERVICE%%N%
)
exit /b 0
...
if not exist "!abs_import!" (
    echo %R%[^^!] ОШИБКА: Файл для импорта конфига не найден: !abs_import!%N%
    exit /b 1
)
echo %C%[*] Импортирую конфиг: %Y%!abs_import!%N%
copy /y "!abs_import!" "!CONFIG_FILE!" >nul
if exist "!DEST_FILE!" (
    echo %C%[*] Проверяю конфигурацию...%N%
    "!DEST_FILE!" -c "!CONFIG_FILE!" check
    if %ERRORLEVEL% neq 0 (
        echo %R%[^^!] ОШИБКА: Проверка конфигурации завершилась ошибкой.%N%
        exit /b 1
    )
    echo %G%[+] Конфигурация корректна.%N%
)
exit /b 0
...
if not exist "!DEST_FILE!" (
    echo %R%[^^!] ОШИБКА: Бинарник не найден: !DEST_FILE!%N%
    exit /b 1
)
...
if /i "!svc_status!"=="Running" (
    echo %Y%[^^!] Служба %RESOLVED_SERVICE% уже запущена. Ручной запуск может вызвать конфликт портов.%N%
    set /p "stop_svc_choice=%C%[?] Остановить службу перед ручным запуском? [y/N]: %N%"
...
echo.
echo %C%[*] Выберите конфигурацию для запуска:%N%
echo   %Y%1%N%) Указать путь к существующему файлу конфигурации
echo   %Y%2%N%) Создать новый файл конфигурации (default_config.json)
echo   %Y%0%N%) Отменить запуск sing-box-extended
echo.

:run_config_prompt
set "r_choice="
set /p "r_choice=%C%[?] Выберите вариант (0-2): %N%"
if "!r_choice!"=="0" (
    echo %G%[*] Запуск отменен.%N%
    exit /b 0
)
if "!r_choice!"=="1" (
    set /p "user_conf_path=%C%[?] Укажите полный путь к config.json: %N%"
    for %%i in ("!user_conf_path!") do set "RUN_CONF=%%~fi"
    if not exist "!RUN_CONF!" (
        echo %R%[^^!] ОШИБКА: Файл конфигурации не найден: !RUN_CONF!%N%
        exit /b 1
    )
    goto do_run
)
if "!r_choice!"=="2" (
    ...
    if not exist "!RUN_CONF!" (
        ...
        echo %G%[+] Создан конфиг: %Y%!RUN_CONF!%N%
    ) else (
        echo %Y%[^^!] Файл уже существует: !RUN_CONF!%N%
    )

    :: Open JSON Editor
    echo %C%[*] Открываю конфиг в редакторе по умолчанию...%N%
    echo %Y%[^^!] Сохраните файл и закройте редактор для продолжения.%N%
    start /wait "" "!RUN_CONF!"
    goto do_run
)
echo %R%[^^!] Неверный выбор.%N%
goto run_config_prompt
...
echo %C%[*] Запускаю sing-box extended...%N%
echo %Y%[^^!] Для остановки нажмите Ctrl+C.%N%
